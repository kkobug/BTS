...
- document DB는 json으로만 저장이 되나요?
  - 아니요, 여기서 Document는 JSON 이나 XML같은 표준 형식을 말합니다.

- 그래서 언제 SQL과NoSQL을 쓰는지 ?
